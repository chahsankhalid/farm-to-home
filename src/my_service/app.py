...
from <%my_service%> import domain, endpoints, persistence
from <%my_service%>.config import AppConfig, ConfigLoader
from <%my_service%>.domain import models
from <%my_service%>.middleware.fraudio_middleware import FraudioMiddleware
from <%my_service%>.openapi import bundling
from <%my_service%>.persistence.cached_hash_results import cached_hash_results
...
from <%my_service%>.serialization.custom_json_encoder import CustomJsonEncoder
from <%my_service%>.util import paths, yaml
from <%my_service%>.util.secrets import build_secrets_manager
from <%my_service%>.util.tracing import FraudioTracingSampler, trace_functions_in_module
...
def create_app() -> AsyncApp:
    ...
    if config.tracing_enabled:
        ...
        trace_functions_in_module(persistence)
    ...
    app_openapi_spec = bundling.get_bundled_specs(paths.app('openapi', 'api-spec.yaml'))
    cnx_app.add_api(app_openapi_spec, strict_validation=True, validate_responses=True)
...
async def lifespan_handler(_: ConnexionMiddleware) -> AsyncIterator:
    logger = logging.getLogger(__name__)
    logger.info(f'App lifespan: setting up async features...')
    config = ConfigLoader.get_instance()
    postgres_enabled = config.fraudio_postgres_enabled
    if postgres_enabled:
        await db.async_setup()
    await data_tables.async_setup()
    await cached_hash_results.async_setup()
    logger.info(f'App lifespan: setup complete.')
    yield
    logger.info(f'App lifespan: tearing down async features...')
    await cached_hash_results.async_teardown()
    await data_tables.async_teardown()
    if postgres_enabled:
        await db.async_teardown()
    logger.info(f'App lifespan: teardown complete.')
...
def main():
    ...
    if ConfigLoader.get_instance().hot_reload:
        ...
        cnx_app.run('<%my_service%>.app:create_app', reload_dirs=reload_dirs)
    else:
        cnx_app.run()
